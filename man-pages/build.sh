...
get_version() {
	...
	local git_ver=$(timeout 2 git ls-remote --tags --refs https://git.kernel.org/pub/scm/docs/man-pages/man-pages.git | grep -E "refs/tags/man-pages-[0-9]+\.[0-9]+\.[0-9]+" | cut -d '/' -f 3 | sed -E 's/man-pages-//g' | sort -V | sed -E 's/^1\.//g' | tail -n 1)
