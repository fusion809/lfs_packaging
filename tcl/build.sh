...
get_version() {
	...
	local up_ver=$(wget -T 2 -t 1 -c https://github.com/tcltk/tcl/releases -qO-  | grep "core-8" | sed 's/.*core-//g' | cut -d '"' -f 1 | sed 's/&amp.*//g' | tr '-' '.' | sort -V | tail -n 1)
	ver_check "$up_ver" "$inst_ver" && return
	local git_ver=$(timeout 2 git ls-remote --tags --refs https://github.com/tcltk/tcl.git | grep -E "core-8-[0-9]-[0-9]+$" | cut -d '/' -f 3 | sed 's/core-//g' | tr '-' '.' | sort -V | tail -n 1)
